...
void VEngine::Graphics::AnimationIO::Read(FILE* file, Animation& animation)
{
    ...
    for (uint32_t k = 0; k < keyCount; ++k)
    {
        ...
        builder.AddPositionKeys(pos, time);
    }
    ...
    for (uint32_t k = 0; k < keyCount; ++k)
    {
        ...
        builder.AddRotationKeys(rot, time);
    }
    ...
    for (uint32_t k = 0; k < keyCount; ++k)
    {
        ...
        builder.AddScaleKeys(scale, time);
    }
...
void VEngine::Graphics::ModelIO::SaveSkeleton(std::filesystem::path filePath, const Model& model)
{
    ...
    for (uint32_t i = 0; i < boneCount; i++)
    {
        ...
        WriteMatrix(boneData->offsettTransform);
    }
    fclose(file);
}

void VEngine::Graphics::ModelIO::LoadSkeleton(std::filesystem::path filePath, Model& model)
{
    filePath.replace_extension("skeleton");
    FILE* file = nullptr;
    fopen_s(&file, filePath.u8string().c_str(), "r");
    ...
    for (uint32_t i = 0; i < boneCount; i++)
    {
        ...
        ReadMatrix(boneData->offsettTransform);
    }
    fclose(file);
}

void VEngine::Graphics::ModelIO::SaveAnimation(std::filesystem::path filePath, const Model& model)
{
...
void VEngine::Graphics::ModelIO::LoadAnimation(std::filesystem::path filePath, Model& model)
{
    filePath.replace_extension("animset");
