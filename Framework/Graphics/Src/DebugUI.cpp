...
#include <../External/ImGui/Inc/imgui_impl_dx11.h>
#include<../External/ImGui/Inc/imgui_impl_win32.h>
...
namespace
{
	...
	bool IsKeyboardImput(UINT message)
	{
		switch (message)
		{
		case WM_CHAR:
	...
	LRESULT CALLBACK DebugUIMessageHandler(HWND window, UINT message, WPARAM wParam, LPARAM lParam)
	{
		...
		if (io.WantCaptureKeyboard && IsKeyboardImput(message))
		{
			return ImGui_ImplWin32_WndProcHandler(window, message, wParam, lParam);
		}
