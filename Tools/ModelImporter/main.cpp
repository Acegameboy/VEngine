	...
	const std::filesystem::path& fileName)
{
	...
	ASSERT(written == texture->mWidth, "Error: failed to extract embedded texxture");
	fclose(file);
}
	...
	uint32_t materialIndex)
{
	...
	if (aiMaterial->GetTexture(textureType, 0, &texturePath) == aiReturn_SUCCESS)
	{
		if (texturePath.C_Str()[0] == '*')
		{
			...
			ASSERT(scene->HasTextures(), "Error: No texture found");
		...
		{
			std::filesystem::path embeddedPath = texturePath.C_Str();
			std::string fileName = args.inputFileName.u8string();
			fileName.erase(fileName.length() - 4);
			fileName += suffix;
			fileName += "_" + std::to_string(materialIndex);
			fileName += embeddedPath.extension().u8string();
...
void SetBoneOffesetTransform(const aiBone* nodeBone, Skeleton& skeleton, const BoneIndexMap& boneIndexMap)
{
	uint32_t boneIndex = GetBoneIndex(nodeBone, boneIndexMap);
	Bone* bone = skeleton.bones[boneIndex].get();
	bone->offsettTransform = ToMatrix4(nodeBone->mOffsetMatrix);
}
...
int main(int argc, char* argv[])
{
	...
	printf("Importing %s...", args.inputFileName.u8string().c_str());
	...
	if (scene->HasMeshes())
	{
		...
		for (uint32_t meshIndex = 0; meshIndex < scene->mNumMeshes; ++meshIndex)
		{
			...
			if (aiMesh->HasBones())
			{
				for (uint32_t b = 0; b < aiMesh->mNumBones; ++b)
				{
					...
					SetBoneOffesetTransform(aiBone, *model.skeleton, boneIndexMap);
				}
			}
		}
		...
		{
			bone->offsettTransform._41 *= args.scale;
			bone->offsettTransform._42 *= args.scale;
			bone->offsettTransform._43 *= args.scale;
	...
	if (scene->HasAnimations())
	{
		...
		for (uint32_t animIndex = 0; animIndex < scene->mNumAnimations; ++animIndex)
		{
			...
			//Enplace_back, adds to the vector and returns last item
			...
			for (uint32_t boneAnimIndex = 0; boneAnimIndex < aiAnim->mNumChannels; ++boneAnimIndex)
			{
				...
				for (uint32_t keyIndex = 0; keyIndex < aiBoneAnim->mNumPositionKeys; ++keyIndex)
				{
					...
					builder.AddPositionKeys(ToVector3(pos.mValue)* args.scale, static_cast<float>(pos.mTime));
				}
				for (uint32_t keyIndex = 0; keyIndex < aiBoneAnim->mNumRotationKeys; ++keyIndex)
				{
					...
					builder.AddRotationKeys(ToQuaternion(rot.mValue), static_cast<float>(rot.mTime));
				}
				for (uint32_t keyIndex = 0; keyIndex < aiBoneAnim->mNumScalingKeys; ++keyIndex)
				{
					...
					builder.AddScaleKeys(ToVector3(scale.mValue), static_cast<float>(scale.mTime));
				}
